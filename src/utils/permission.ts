 ...
 */
function can(
  roleOrPermission: string | any[],
  target: { permission: any; mode: any }
) {
  let hasRole = false
  const { permission, mode } = target
  if (mode === 'allOf')
    hasRole = permission.every((item: any) => roleOrPermission.includes(item))
  if (mode === 'oneOf')
    hasRole = permission.some((item: any) => roleOrPermission.includes(item))
  if (mode === 'except')
    hasRole = !permission.every((item: any) => roleOrPermission.includes(item))
  return hasRole
}
