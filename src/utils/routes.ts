...
export function filterRoutes(
  ...
  return routes
    .filter((route: { meta: { guard: any } }) =>
      rolesControl && route.meta && route.meta.guard
        ? hasPermission(route.meta.guard)
        : true
    )
    .flatMap((route: { children: any; meta: { levelHidden: any } }) =>
      baseUrl !== '/' && route.children && route.meta && route.meta.levelHidden
        ? [...route.children]
        : route
    )
    .map((route: any) => {
      ...
      if (route.children && route.children.length > 0) {
        ...
        if (route.children.length > 0) {
          route.childrenPathList = route.children.flatMap(
            (_: { childrenPathList: any }) => _.childrenPathList
          )
          if (!route.redirect)
 ...
 * @returns {*} matched
 */
export function handleMatched(routes: any, path: any) {
  return routes
    .filter(
      (route: { childrenPathList: string | any[] }) =>
        route.childrenPathList.indexOf(path) + 1
    )
    .flatMap((route: { children: any }) =>
 ...
 * @param tag route页信息
 */
export function handleTabs(tag: any) {
  let parentIcon = null
  if (tag.matched)
 ...
 * @returns {string|*}
 */
export function handleActivePath(
  route: { matched?: any; query?: any; path: any; meta?: any },
  isTab = false
) {
 ...
 * @returns {*} Name数组
 */
export function getNames(routes: any) {
  return routes.flatMap((route: { name: any; children: any }) => {
    const names = []
